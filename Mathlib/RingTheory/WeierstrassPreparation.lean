...
open Polynomial.IsDistinguishedAt

lemma preparation_lift_triv {n : ℕ} (neq0 : n = 0) [m.IsMaximal] (f : (R ⧸ m ^ (n + 1))⟦X⟧)
  ...
  have muleq : f = ((Polynomial.X (R := (R ⧸ m ^ (n + 1))) ^ k.toNat) : (R ⧸ m ^ (n + 1))[X]) *
    ...
    simp only [Polynomial.coe_pow, Polynomial.coe_X, order_eq]
  use Unit_of_divided_by_X_pow_order f
  constructor
  · use Polynomial.X ^ k.toNat
    refine ⟨⟨⟨?_⟩, Polynomial.monic_X_pow k.toNat⟩, muleq⟩
  ...
  · rintro h' ⟨g', distinguish, eq⟩
    have : g' = Polynomial.X ^ k.toNat := by
      ext i
      simp only [Polynomial.coeff_X_pow]
      have : constantCoeff h' ∉ m.map (Ideal.Quotient.mk (m ^ (n + 1))) := by
        simpa only [eq_bot', mem_bot] using (h'.1.isUnit_constantCoeff h'.isUnit).ne_zero
      have deg_eq : g'.natDegree = k :=
        distinguish.coe_natDegree_eq_order_map f h' this eq
      have deg_eq' : g'.natDegree = k.toNat := by simp only [← deg_eq, ENat.toNat_coe _]
      by_cases H' : i = k.toNat
    ...
    rw [muleq, this] at eq
    exact Units.ext_iff.mpr ((mul_right_inj' (by simp)).mp eq.symm)

lemma PowerSeries.order_eq_of_eq_zero_iff {R S : Type*} [Semiring R] [Semiring S]
    {φ : PowerSeries R} {ψ : PowerSeries S} (h : ∀ i, coeff i φ = 0 ↔ coeff i ψ = 0) :
    φ.order = ψ.order := by
...
lemma map_ne_top {n : ℕ} (npos : n > 0) (mne : m ≠ ⊤) : m.map (Ideal.Quotient.mk (m ^ n)) ≠ ⊤ := by
...
lemma preparation_lift {n : ℕ} (npos : n > 0) [m.IsMaximal] (f : PowerSeries (R ⧸ m ^ n))
    ...
    let nontriv_all {k : ℕ} (pos : k > 0): Nontrivial (R ⧸ m ^ k) :=
      Submodule.Quotient.nontrivial_iff.mpr (lt_of_le_of_lt
      (Ideal.pow_le_self (Nat.ne_zero_of_lt pos)) (Ne.lt_top IsPrime.ne_top')).ne
    induction n with
    | zero => exact ((Nat.not_lt_zero 0) npos).elim
    | succ n ih =>
      by_cases neq0 : n = 0
      · exact preparation_lift_triv neq0 f ntriv
      · have ord_eq : (f.map (Ideal.Quotient.mk (m.map (Ideal.Quotient.mk (m ^ (n + 1)))))).order =
          ...
          simp [eq_zero_iff_mem, ← map_mk_comap_factorPow m (Nat.zero_lt_of_ne_zero neq0) n.le_succ]
        have : ¬ (f.map (factorPow m n.le_succ)).map
          (Ideal.Quotient.mk (m.map (Ideal.Quotient.mk (m ^ n)))) = 0 := by
          rw [← order_eq_top.not, ← ord_eq, order_eq_top.not]
          exact ntriv
        ...
        let α := trunc k.toNat c
        let γ := (PowerSeries.mk fun i ↦ coeff (i + k.toNat) c)
        ...
        have heq : (α : (R ⧸ m ^ (n + 1))⟦X⟧) + (X ^ k.toNat) * γ = c := by
          ext i
          simp only [coeff_trunc, map_add, Polynomial.coeff_coe, α, coeff_X_pow_mul', coeff_mk, γ]
          by_cases lt : i < k.toNat
        ...
        have : constantCoeff h ∉ m.map (Ideal.Quotient.mk (m ^ n)) := by
          by_contra mem
          absurd (Ideal.eq_top_of_isUnit_mem _ mem (isUnit_iff_constantCoeff.mp h.isUnit))
          exact map_ne_top (Nat.zero_lt_of_ne_zero neq0) Ideal.IsPrime.ne_top'
        have deg' : g.degree = k := by
          rw [Polynomial.degree_eq_natDegree distinguish.monic.ne_zero,
            distinguish.coe_natDegree_eq_order_map _ h this eq, ← ord_eq]
        have deg' : g.degree = k.toNat := by
          rw [deg', eq_comm, ENat.coe_toNat_eq_self]
          exact LT.lt.ne_top (order_finite_iff_ne_zero.mpr ntriv)
        have deg'' : g'.degree = k.toNat := by rw [deg, deg']
        have deg''' : (g' + α).degree = k.toNat := by
          rw [Polynomial.degree_add_eq_left_of_degree_lt (deg'' ▸ (degree_trunc_lt c k.toNat)),
            deg'']
        have mon' : (g' + α).Monic := mon.add_of_left (deg'' ▸ (degree_trunc_lt c k.toNat))
        have αcoeff (l : ℕ) : (factorPow m n.le_succ) (α.coeff l) = 0 := by
          simp only [coeff_trunc, α]
          by_cases lt : l < k.toNat
        ...
        have hgα {i : ℕ} (hi : i < (g' + α).natDegree) :
          ...
          exact hi
        have hgcoeff (l : ℕ): (g.coeff l - if l = k.toNat then 1 else 0) ∈
          m.map (Ideal.Quotient.mk (m ^ n)) := by
          by_cases leq : l = k.toNat
        ...
        have hcoeff (l : ℕ): ((((g' + α) : (R ⧸ m ^ (n + 1))⟦X⟧) - X ^ k.toNat).coeff l) ∈
        ...
        have mul0 : (((g' + α)  : (R ⧸ m ^ (n + 1))⟦X⟧) - (X ^ k.toNat)) * γ = 0 := by
          ext
          ...
          have : γ.coeff x.2  ∈ (m ^ n).map (Ideal.Quotient.mk (m ^ (n + 1))) := by
        ...
        have muleq : (g' + α) * (h'.1 * (1 + γ)) = f := by
          ...
          _ = (g' : (R ⧸ m ^ (n + 1))⟦X⟧) * h'.1 + h'.1 * ((α : (R ⧸ m ^ (n + 1))⟦X⟧) +
              X ^ k.toNat * γ) + (((g' + α)  : (R ⧸ m ^ (n + 1))⟦X⟧) - X ^ k.toNat) * γ * h'.1 := by
              ring
        ...
        have : constantCoeff H ∉ m.map (Ideal.Quotient.mk (m ^ (n + 1))) := by
          by_contra mem
          absurd (Ideal.eq_top_of_isUnit_mem _ mem (isUnit_iff_constantCoeff.mp H.isUnit))
          exact map_ne_top n.zero_lt_succ Ideal.IsPrime.ne_top'
        have degG' : G.degree = k.toNat := by
          rw [Polynomial.degree_eq_natDegree distinguishG.monic.ne_zero,
            distinguishG.coe_natDegree_eq_order_map f H this eq', eq_comm, ENat.coe_toNat_eq_self]
          exact (order_finite_iff_ne_zero.mpr ntriv).ne_top
        ...
        have hcoeff' (l : ℕ): ((G  : (R ⧸ m ^ (n + 1))⟦X⟧) - X ^ k.toNat).coeff l ∈
          m.map (Ideal.Quotient.mk (m ^ (n + 1))) := by
          simpa [← map_mk_comap_factorPow m (Nat.zero_lt_of_ne_zero neq0) n.le_succ, coeff_X_pow,
            ← (Polynomial.coeff_map (factorPow m n.le_succ) l), mapeq'] using hgcoeff l
        have mul0' : ((G  : (R ⧸ m ^ (n + 1))⟦X⟧) - (X ^ k.toNat)) * (H.1 - h'.1) = 0 := by
          ext
          simp only [coeff_mul, map_zero]
          apply Finset.sum_eq_zero fun x _ => ?_
          ...
          have : (H.1 - h'.1).coeff x.2  ∈ (m ^ n).map (Ideal.Quotient.mk (m ^ (n + 1))) := by
          ...
          exact Submodule.mul_mem_mul hr hs
        have eq'' : f = (g'  : (R ⧸ m ^ (n + 1))⟦X⟧) * h'.1 + (X ^ k.toNat) * (H.1 - h'.1) +
          ((G - g') : (R ⧸ m ^ (n + 1))⟦X⟧) * h'.1 := by
          calc
          _= G * H.1 := by rw [eq']
          _= (g' : (R ⧸ m ^ (n + 1))⟦X⟧) * h'.1 +
             ((G : (R ⧸ m ^ (n + 1))⟦X⟧) - (X ^ k.toNat)) * (H.1 - h'.1) +
             (X ^ k.toNat) * (H.1 - h'.1) + ((G - g') : (R ⧸ m ^ (n + 1))⟦X⟧) * h'.1 := by ring
          _= _ := by simp [mul0']
        have c_decomp : c = X ^ k.toNat * ((H.1 - h'.1) * h'.inv) +
          ((G - g') : (R ⧸ m ^ (n + 1))⟦X⟧) := by
          calc
          _= h'.inv * (f - ↑g' * h'.1) := rfl
          _= h'.inv * ((g'  : (R ⧸ m ^ (n + 1))⟦X⟧) * h'.1 + (X ^ k.toNat) * (H.1 - h'.1) +
            ((G - g') : (R ⧸ m ^ (n + 1))⟦X⟧) * h'.1 - (g' : (R ⧸ m ^ (n + 1))⟦X⟧) * h'.1) := by
            congr
          _= X ^ k.toNat * ((H.1 - h'.1) * h'.inv) +
            ((G - g') : (R ⧸ m ^ (n + 1))⟦X⟧) * h'.1 * h'.inv := by ring
          _= _ := by rw [mul_assoc, h'.val_inv, mul_one]
        have coeff_ge {l : ℕ} (lge : l ≥ k.toNat): G.coeff l - g'.coeff l = 0 := by
          ...
          · have lgt : l > k.toNat := Nat.lt_of_le_of_ne lge fun a ↦ leq a.symm
        ...
        have eqγ : ((H.1 - h'.1) * h'.inv) = γ := by
          ...
          simp [coeff_ge (Nat.le_add_left k.toNat i)]
        simp [mul_add, ← eqγ, mul_comm (H.1 - h'.1) _, ← mul_assoc]

theorem CompleteLocalRing.weierstrass_preparation [m.IsMaximal] [comp : IsAdicComplete m R]
  ...
  let R_ntriv : Nontrivial R := nontrivial_of_ne 0 1 (ne_of_mem_of_not_mem (Submodule.zero_mem m)
    ((Ideal.ne_top_iff_one m).mp IsPrime.ne_top'))
  let R_ntriv' {k : ℕ} (kpos : k > 0): Nontrivial (R ⧸ m ^ k) :=
    Submodule.Quotient.nontrivial_iff.mpr (lt_of_le_of_lt
      (Ideal.pow_le_self (Nat.ne_zero_of_lt kpos)) IsPrime.ne_top'.lt_top).ne
  have map_order (n : ℕ+) : ((f.map (Ideal.Quotient.mk (m ^ n.1))).map
      (Ideal.Quotient.mk (m.map (Ideal.Quotient.mk (m ^ n.1))))).order =
      (f.map (Ideal.Quotient.mk m)).order := by
    apply PowerSeries.order_eq_of_eq_zero_iff (fun i ↦ ?_)
    simp [eq_zero_iff_mem, pow_le_self (Nat.ne_zero_of_lt n.2)]
  have (n : ℕ+) : ((f.map (Ideal.Quotient.mk (m ^ n.1))).map
      (Ideal.Quotient.mk (m.map (Ideal.Quotient.mk (m ^ n.1))))) ≠ 0 := by
    rw [← order_finite_iff_ne_zero, map_order n, order_finite_iff_ne_zero]
    exact ntriv
  ...
  have series_deg (n : ℕ+) : (g_series' n).degree = k := by
    ...
    have : constantCoeff (h_series' n) ∉ m.map (Ideal.Quotient.mk (m ^ n.1)) := by
      by_contra mem
      absurd (Ideal.eq_top_of_isUnit_mem _ mem (isUnit_iff_constantCoeff.mp (h_series' n).isUnit))
      exact map_ne_top n.2 Ideal.IsPrime.ne_top'
    rw [Polynomial.degree_eq_natDegree (series_distinguish n).monic.ne_zero,
      (series_distinguish n).coe_natDegree_eq_order_map _ _ this (series_eq n), map_order n]
  have series_deg' (n : ℕ+) : (g_series' n).degree = k.toNat := by
    rw [series_deg, eq_comm, ENat.coe_toNat_eq_self]
    exact LT.lt.ne_top (order_finite_iff_ne_zero.mpr ntriv)
  ...
  have h_series_factorPoweq' {a b : ℕ} (apos : a > 0) (bpos : b > 0) (le : a ≤ b) :
    ...
    · refine ⟨⟨fun {i} hi ↦ ?_⟩, (series_distinguish ⟨b, bpos⟩).monic.map (factorPow m le)⟩
      ...
      simpa [← mem_comap,  map_mk_comap_factorPow m apos le] using
  ...
  have h_series_factorPoweq {a b : ℕ} (apos : a > 0) (bpos : b > 0) (le : a ≤ b) :
    ...
    Units.ext_iff.mp (h_series_factorPoweq' apos bpos le)
  ...
  have h_coeff_series_mod (i : ℕ): ∀ {a b : ℕ}, a ≤ b → (h_series a).coeff i  ≡
    (h_series b).coeff i [SMOD m ^ a • (⊤ : Submodule R R)] := by
    intro a b le
    by_cases apos : a > 0
  ...
  have h_spec (i : ℕ) : ∀ n : ℕ, (h_series n).coeff i ≡ h.coeff i
  ...
  have hu : IsUnit h := by
    rw [isUnit_iff_constantCoeff, isUnit_iff_notMem_of_isAdicComplete_maximal m]
    by_contra mem
  ...
  have g_coeff_series_mod (i : ℕ) : ∀ {a b : ℕ}, a ≤ b → (g_series a).coeff i ≡
    ...
    · simp [Nat.eq_zero_of_not_pos apos]
  let g_coeff : ℕ → R := fun i ↦ if i = k.toNat then 1 else if i > k.toNat then 0
    else Classical.choose (IsPrecomplete.prec IsAdicComplete.toIsPrecomplete (g_coeff_series_mod i))
  have lt {i : ℕ}: g_coeff i ≠ 0 → i < k.toNat + 1 := by
    intro ne0
    by_contra gt
    ...
    toFinsupp := {
      support :=
        have : Fintype {i | g_coeff i ≠ 0} :=
          Fintype.ofInjective (fun i ↦ (⟨i.1, lt i.2⟩ : Fin (k.toNat + 1))) (fun i j hij ↦
            Subtype.val_inj.mp <| Fin.mk.inj_iff.mp hij)
        {i | g_coeff i ≠ 0}.toFinset
      toFun := g_coeff
      mem_support_toFun := by simp }}
  have g_spec {i : ℕ} (hi : i < k.toNat) : ∀ n : ℕ, (g_series n).coeff i ≡ g_coeff i
    [SMOD m ^ n • (⊤ : Submodule R R)] := by
  ...
  have g_spec' {n : ℕ} (npos : n > 0) : g.map (Ideal.Quotient.mk (m ^ n)) =
    ...
    have deg : (g_series n).degree = k.toNat := by
      rw [← series_deg' ⟨n, npos⟩, (g_series_spec npos).2.1]
    have ndeg := Polynomial.natDegree_eq_of_degree_eq_some deg
    ext i
    simp only [Polynomial.coeff_map]
    by_cases ne : i = k.toNat
  ...
  constructor
  · use g
    have degeq : g.natDegree = k.toNat := by
      apply Polynomial.natDegree_eq_of_le_of_coeff_ne_zero ?_ (by simp [g, g_coeff])
      rw [g.natDegree_le_iff_degree_le,  g.degree_le_iff_coeff_zero k.toNat]
      intro i hi
      have lt : k.toNat < i := WithBot.coe_lt_coe.mp hi
    ...
    · simp only [IsUnit.unit_spec]
      ...
      ext i
      have modeq (n : ℕ) : f.coeff i - (g * h).coeff i ≡ 0
        [SMOD m ^ n • (⊤ : Submodule R R)] := by
        by_cases npos : n > 0
        · simp [← (mk_eq_mk_iff_sub_mem _ _), ← coeff_map, this npos, SModEq.zero]
        · simp [Nat.eq_zero_of_not_pos npos]
      rw [← sub_eq_zero,
        (IsAdicComplete.toIsHausdorff.haus (f.coeff i - (g * h).coeff i) modeq)]
  · rintro H ⟨G, distinguishG, eq⟩
    ...
    have modeq' {n : ℕ} (npos : n > 0) : (Hu n).unit = (h_series' ⟨n, npos⟩) := by
      ...
      have degeq : G.degree = k.toNat := by
        have : constantCoeff H ∉ m := by
          by_contra mem
          absurd (Ideal.eq_top_of_isUnit_mem _ mem (isUnit_iff_constantCoeff.mp H.isUnit))
          exact IsPrime.ne_top'
        rw [Polynomial.degree_eq_natDegree distinguishG.monic.ne_zero,
          distinguishG.coe_natDegree_eq_order_map f H this eq,
          eq_comm, ENat.coe_toNat_eq_self]
        exact (order_finite_iff_ne_zero.mpr ntriv).ne_top
      have degmapeq : (G.map (Ideal.Quotient.mk (m ^ n))).degree = k.toNat := by
    ...
    ext i
    simp only [IsUnit.unit_spec]
    have coeff_modeq' (n : ℕ): H.1.coeff i  ≡ h.coeff i [SMOD m ^ n • (⊤ : Submodule R R)] := by
      by_cases npos : n > 0
    ...
    exact sub_eq_zero.mp <| IsHausdorff.haus IsAdicComplete.toIsHausdorff
      (H.1.coeff i - h.coeff i) (fun n ↦ SModEq.zero.mpr (SModEq.sub_mem.mp (coeff_modeq' n)))
...
lemma IsDiscreteValuationRing.weierstrass_preparation_aux [IsDomain R] [m.IsMaximal]
  ...
  classical
  have exist_nmem : ∃ n : ℕ, ∃ i, f.coeff i ∉ m ^ n := by
    by_contra! h
    absurd ne0
    ext i
    have (n : ℕ) : f.coeff i ≡ 0 [SMOD m ^ n • (⊤ : Submodule R R)] := by
      simp [SModEq.zero, h n i]
    simp [IsAdicComplete.toIsHausdorff.haus (f.coeff i) this]
  ...
  have : ∀ i, f.coeff i ∈ Ideal.span {π ^ k} := by
  ...
  have muleq {g : R⟦X⟧} : (π ^ k) • g = f → g = f' := by
    intro eq
    ext i
    have : (π ^ k • g).coeff i = (π ^ k • f').coeff i := by rw [eq, f'_spec]
  ...
  have : Nat.find exist_nmem = k' + 1 := by
    ...
    · use g'.natDegree
      ...
      have nmem : (g' * h'.1).coeff g'.natDegree ∉ m := by
        simpa [← eq_zero_iff_mem.not, ← coeff_map, map_mul, ← Polynomial.polynomial_map_coe,
          map_eq_X_pow distinguish', Polynomial.coe_pow, Polynomial.coe_X, coeff_X_pow_mul']
